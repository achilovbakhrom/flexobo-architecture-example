...
@Injectable()
export class SubscriptionAggregateStore implements IAggregateStore<SubscriptionAggregate> {
  private readonly logger = new Logger(SubscriptionAggregateStore.name);

  constructor(private readonly prisma: PrismaService) {}

  async load(id: string): Promise<SubscriptionAggregate | null> {
    const events = await this.prisma.event.findMany({
      where: {
        aggregateId: id,
        aggregateType: AggregateType.SUBSCRIPTION,
      },
      orderBy: { version: 'asc' },
    });
    ...
    return SubscriptionAggregate.fromEvents(domainEvents);
  }
  ...
  async save(aggregate: SubscriptionAggregate): Promise<DomainEvent[]> {
      ...
      for (const event of uncommittedEvents) {
        ...
        await tx.outbox.create({
          data: {
            ...
            routingKey: `billing.subscription.${event.type.toLowerCase().replace(/_/g, '.')}`,
          },
        });
      }
    });
    ...
    this.logger.debug(
      `Saved ${uncommittedEvents.length} events for subscription ${aggregate.id}`,
    );
