...
export class ActivateLoadCommand implements ICommand {
  constructor(public readonly loadId: string, public readonly userId: string) {}
}

@CommandHandler(ActivateLoadCommand)
export class ActivateLoadHandler
  implements ICommandHandler<ActivateLoadCommand, void>
{
  constructor(
  ...
  async execute(command: ActivateLoadCommand): Promise<Result<void, Error>> {
    try {
