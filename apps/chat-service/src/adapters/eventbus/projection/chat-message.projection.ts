...
import { CHAT_MESSAGE_NOTIFICATION_RESOLVER } from '../notification-resolvers';
...
> {
  constructor(
    ...
    @Optional()
    @Inject(CHAT_MESSAGE_NOTIFICATION_RESOLVER)
    notificationResolver: INotificationResolver<ChatMessageEventPayload> | null
  ) {
    super(rabbitMQConsumer, ChatMessageProjection.name, eventBuffer, messagePublisher, notificationResolver);
  }
